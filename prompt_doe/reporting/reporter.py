...
class AnalysisReport:
    ...
    def generate(
        ...
        """
        Generate a complete AnalysisReport from ExperimentResults.
        """
        ...
        optimal_rec = OptimalPromptFinder.find_optimal_prompt(anova_result=anova_res)
    ...
    def to_markdown(self, save_path: Optional[Union[str, Path]] = None) -> str:
        ...
        if self.anova_result:
            for eff in self.anova_result.main_effects:
                ...
                action_badge = "🟢 ENABLE" if eff.recommendation == "enable" else ("🔴 DISABLE" if eff.recommendation == "disable" else "⚪ OMIT/DEFAULT")
                lines.append(
                    ...
                    f"{eff.cohen_d:.2f} | {sig_badge} | {action_badge} |"
                )
        ...
        if self.alias_structure:
            lines.extend([
                ...
                f"",
                f"- **Design Resolution:** `{self.alias_structure.resolution_name}`",
    ...
    def to_html(self, save_path: Optional[Union[str, Path]] = None) -> str:
        ...
        # Convert markdown tables to HTML or wrap in clean CSS
        html = f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
