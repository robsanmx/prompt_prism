...
class ResponseCache:
    ...
    def __init__(self, db_path: Optional[Union[str, Path]] = None, enabled: bool = True):
    ...
    def _hash_key(self, prompt: Any, params: Dict[str, Any]) -> str:
        serialized = json.dumps({"prompt": prompt, "params": params}, sort_keys=True, default=str)
        return hashlib.sha256(serialized.encode("utf-8")).hexdigest()

    def get(self, prompt: Any, params: Optional[Dict[str, Any]] = None) -> Optional[LLMResponse]:
        if not self.enabled:
            return None
        ...
        if self.db_path and self.db_path.exists():
            with sqlite3.connect(self.db_path) as conn:
                cur = conn.cursor()
                cur.execute("SELECT content, raw_json, latency_ms, token_usage_json FROM response_cache WHERE prompt_hash = ?", (h,))
                row = cur.fetchone()
                if row:
                    ...
                    res = LLMResponse(content=content, raw=raw, latency_ms=latency_ms, token_usage=tokens)
                    self._memory_cache[h] = res
                    return res

        return None

    def set(self, prompt: Any, response: LLMResponse, params: Optional[Dict[str, Any]] = None) -> None:
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute(
                    ...
                    (
                        h,
                        response.content,
                        json.dumps(response.raw, default=str) if response.raw is not None else "",
                        response.latency_ms,
